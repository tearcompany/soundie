...
- **`returnEngine.logVisit`** — pierwsza wizyta dnia (kalendarz `YYYY-MM-DD` z klienta), nalicza `streakNights`, `first_visit` / `second_day_return` w analityce; odpowiedź: `streakNights`, szept z aktywnej nuty (`noteShort`), `isFirstVisitEver`, `shouldShowWelcomeBack` (nadal: tylko drugi kalendarzowy dzień użycia), **`returnStory`**: `none` | `first_day` | `second_day` | `returning` (kontekst narracji na dany dzień: pierwsza sesja, most do wczoraj, dalsza seria)
...
- **Return Engine (tydz. 1 + 2)** — `ReturnEngineBridge`: `logVisit` + `revealDailyClaim`; przy `isNew` tylko **`DailyGiftDialog`** — narracja wg `returnStory` / `streakNights` (bez osobnego welcome); persist dnia w `sessionStorage`: `reLog`, `reCtx` (`returnStory`, seria, szept); warianty copy `returnEngine.dailyGift.*`; Teardrop + caption + glow; **Listen**
